...
const JobBreakdown = () => {
  ...
  const [steps, setSteps] = useState<JobStep[]>([
    { id: 1, stepNumber: 1, description: "", attachments: [] },
  ]);
  ...
  const handleContinue = () => {
    const emptySteps = steps.filter((step) => !step.description.trim());
    if (emptySteps.length > 0) {
      toast.error("Please fill in all step descriptions");
      return;
    }
    toast.success("Proceeding to Hazard Identification");
    navigate("/hazard-identification");
  };

  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Job Breakdown / Job Steps Builder</h1>
        <p className="text-muted-foreground">Break down the job into sequential steps</p>
      </div>
